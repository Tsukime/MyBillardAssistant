...
public class MainActivity extends Activity implements OnTouchListener, CvCameraViewListener2 {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mButtons[1].setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
		    	...
		    	double rougeX   = listeBoules.get(1).second.first;
		    	double rougeY   = listeBoules.get(1).second.second;
		    	double jauneX   = listeBoules.get(0).second.first;
		    	double jauneY   = listeBoules.get(0).second.second;
		    	
		    	
		    	// changement de contentView
		    	//setContentView(R.layout.activity_sol);
		    	//ImageButton btnRtn = (ImageButton)findViewById(R.id.imageButton9);
		    	//@SuppressWarnings("unused")
				//View view = (View)findViewById(R.id.view);
		    	canvas = new ShowSol(_this);
		    	...
		    	s.execute();
				//double[] meilleurCoup = Simulation.MeilleurCoup(280, blancheX, blancheY, jauneX, jauneY, rougeX, rougeY);
				//positions += "Puissance : " + df.format(meilleurCoup[0]) + "\n";
		    	//positions += "Angle : " + df.format(meilleurCoup[1]) + "\n";
		    	
		    	
		    	
				/*for (int i=0; i<3; i++){
					positions += listeBoules.get(i).first + " : " + df.format(listeBoules.get(i).second.first) + " , " + df.format(listeBoules.get(i).second.second);
					if (i<2){positions += "\n";}
				}
    ...
    public boolean onTouch(View v, MotionEvent event) {
    	System.out.println("MainActivity : onTouch Screen");    		
    ...
    private boolean track(){
    	...
    	while (!trouve){
	    	...
	    	int c = 0;
	    	//Pour determiner le contour qui a l'aire maximale, quand on a trouve plusieurs contours
	    	double max = 0.;
	        mDetector.process(mRgba);
    ...
    private boolean perspectiveTransform(){
         ...
         * Dans le cas ou on a les 4 coins on projette les 4 coins sur les 4 coins finales
    ...
    private void takePicture(Mat mat, String name){
		...
		// a reparer, ancienne version 2.4 de openCV (on est a la 3.2, n'existe plus)
    ...
    public ArrayList<Point> computeIntersections(ArrayList<Point> liste){
        ...
        int xOffset = (mOpenCvCameraView.getWidth() - cols);
        int yOffset = (mOpenCvCameraView.getHeight() - rows);
    ...
    /*
     * Retourne la liste des boules avec leurs coordonnees respectives
     */
